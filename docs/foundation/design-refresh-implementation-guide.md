...
> **ステータス：使用停止（2026-08-15）**
> 対象としていた既存8案が取り下げられたため、本手順書に基づく新規実装や本番反映は開始しません。内容は履歴として保持します。`docs/foundation/concept.md` の合意後、カラーパレットとフォントを含む複数の完全な新規案について、比較条件と実装手順を改めて作成します。
